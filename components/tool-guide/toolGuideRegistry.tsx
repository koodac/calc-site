...
function FactorGuide() {
  return (
    <>
      ...
      <GuideFaq
        items={[
          { q: "판별식이 음수면 인수분해가 불가한가요?", a: "실수 계수 범위에서는 불가합니다. 복소수 범위까지 확장하면 (x − α)(x − ᾱ) 형태로 분해할 수 있습니다." },
          { q: "삼차 이상 다항식도 인수분해가 되나요?", a: "네. 삼차식은 먼저 유리근 정리(p/q 후보 대입)로 한 근을 찾은 뒤 조립제법으로 이차식으로 낮춰 인수분해합니다." },
        ]}
      />
    </>
  );
